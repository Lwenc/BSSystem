...
namespace HASystem
{
    ...
    public class CRC16
    {
     readonly byte[] CRCHigh = new byte[]{  
       ...
       public byte[] GetCRC16(byte[] Cmd, bool IsHighBefore)  
       {  
           int index;  
           int crc_Low = 0xFF;  
            int crc_High = 0xFF;  
    ...
    public static partial class CRC32
    {
        ...
        public static string GetCrc32(string command)
        {

            byte[] buffer = strToByteByBase(command, 16);
            byte buffresult = 0;
            ...
            return ASCIIStrToHexStr((~crcValue).ToString("X8"));
        }

        private static byte[] strToByteByBase(string hexString, int fromBase)
        {
            ...
            for (int index = 0; index < numArray.Length; ++index)
                numArray[index] = Convert.ToByte(hexString.Substring(index * 2, 2).Trim(), fromBase);
            return numArray;
        }

        private static string ASCIIStrToHexStr(string asciiString)
        {
            string str = (string)null;
            if (asciiString != null)
            {
                for (int index = 0; index < asciiString.Length; ++index)
                    str += ((byte)asciiString[index]).ToString("X2");
            }
            return str;
        }
